...
브라우저가 루트의 HTML, CSS와 의존성 없는 소형 JavaScript를 직접 읽는 정적 사이트다. 패키지 관리자, 빌드 단계, 런타임 의존성은 없다. 상대 경로를 사용하므로 GitHub Pages에서 단순하게 제공할 수 있다. `theme.js`는 테마 버튼의 상태와 `localStorage`에 저장한 사용자 선택만 관리한다.
